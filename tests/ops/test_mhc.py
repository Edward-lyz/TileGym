...
class Test_MHC(common.PyTestCase):
    ...
    def test_mhc_gemm_rms_scale_bf16_precision(self, m, k, n, backend):
    ...
    @pytest.mark.parametrize("backend", _backends)
    def test_mhc_sinkhorn(self, m, n, dtype, backend):
        if tilegym.is_backend_available(backend):
            tilegym.set_backend(backend)
        else:
            pytest.skip(f"Backend {backend} is not available")
    ...
    @pytest.mark.parametrize("backend", _backends)
    def test_mhc_apply_residual(self, m, n, c, backend):
        if tilegym.is_backend_available(backend):
            tilegym.set_backend(backend)
        else:
            pytest.skip(f"Backend {backend} is not available")
        ...
        x = torch.randn((m, nC), dtype=X_DTYPE, device=device, requires_grad=False) * 10
        f_out = torch.randn((m, c), dtype=X_DTYPE, device=device, requires_grad=False) * 10
        y = torch.randn((m, out_n), dtype=OTHER_DTYPE, device=device, requires_grad=False) * 10
